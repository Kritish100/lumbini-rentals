...
export default function PropertyForm({
  ...
  useEffect(() => {
    if (editingId) {
      ...
      reset({
        ...
        specifications: Object.entries(property?.specifications || {}).map(
          ([key, value], i) => ({
            id: i.toString(),
            key,
            value,
  ...
  const onSubmit = async (data: PropertyFormValues) => {
    const payload: Partial<AdminProperty> = {
      ...
      specifications: Object.assign(
        {},
        ...data.specifications.map((i) => ({ [i.key]: i.value })),
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
        ...
        <div className="flex items-center">
          <button
            ...
            className="flex items-center text-xs font-bold text-slate-400 hover:text-slate-700 uppercase"
          >
          ...
          {editingId && (
            <button
              type="button"
              onClick={onCancel}
              className="text-xs ml-3 font-bold text-slate-400 hover:text-slate-700 uppercase"
            >
              Cancel Edit
            </button>
          )}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest bg-slate-50 p-2 rounded-md">
            1. Basic Info
          </h3>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-600 mb-1">
              Owner Details <span className="text-red-500">*</span>
            </label>
